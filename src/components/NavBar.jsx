...
const NavBar = () => {
  ...
  const handleMenuClick = () => {
    if (window.innerWidth < 640) {
      // Verifica si el tamaño de pantalla es menor a 'sm'
      setMenuOpen(false)
    }
  }
  ...
  return (
    <>
      <nav
        className="fixed z-20 px-[5%] lg:px-[15%] w-full flex justify-between items-center p-3 sm:p-4 transition font-bold"
        style={bgStyle}
      >
        <h1 className="text-5xl italic font-normal">Úrsula Castro</h1>
        <button
          className="block lg:hidden text-zinc-900 rounded focus:outline-none"
